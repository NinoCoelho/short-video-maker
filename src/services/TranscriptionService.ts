...
import { YouTubeSubtitleDownloader, SubtitleDownloadOptions } from './YouTubeSubtitleDownloader';
...
export class TranscriptionService extends EventEmitter {
  ...
  private subtitleDownloader: YouTubeSubtitleDownloader;
  ...
  constructor(dataDir: string, ffmpeg?: FFmpeg) {
    ...
    this.subtitleDownloader = new YouTubeSubtitleDownloader();
  ...
  public async transcribeYouTubeVideo(
    ...
    try {
      ...
      logger.info({ url }, 'Attempting to download YouTube subtitles');
      this.updateProgress(jobId, 10);
      
      const subtitleOptions: SubtitleDownloadOptions = {
        language: transcriptionOptions.language || 'en',
        preferAutoGenerated: true,
        format: 'srt'
      };

      const subtitleResult = await this.subtitleDownloader.downloadSubtitles(url, subtitleOptions);
      
      if (subtitleResult.success && subtitleResult.segments) {
        logger.info({ 
          url, 
          language: subtitleResult.language,
          segmentCount: subtitleResult.segments.length,
          isAutoGenerated: subtitleResult.isAutoGenerated
        }, 'Successfully downloaded YouTube subtitles');
        
        this.updateProgress(jobId, 50);

        // Create a transcription result from subtitles
        const result: TranscriptionResult = {
          text: subtitleResult.segments.map(s => s.text).join(' '),
          segments: subtitleResult.segments,
          language: subtitleResult.language,
          languageCode: subtitleResult.language,
          duration: subtitleResult.segments[subtitleResult.segments.length - 1]?.endTime || 0,
          wordCount: subtitleResult.segments.reduce((count, seg) => count + seg.text.split(/\s+/).length, 0),
          avgConfidence: 1.0, // Subtitles have perfect confidence
          processingTime: Date.now() - startTime,
          modelUsed: 'youtube-subtitles',
          provider: 'youtube'
        };
        ...
        // Store result for future use
        const audioHash = crypto.createHash('sha256').update(url).digest('hex');
        const transcriptionStorage: TranscriptionStorage = {
          id: this.generateStorageId(),
          audioFilePath: url,
          audioFileHash: audioHash,
        ...
        await this.storage.store(transcriptionStorage);
        const cacheKey = this.generateCacheKey(audioHash, transcriptionOptions);
        await this.cache.set(cacheKey, transcriptionStorage, 3600);
        ...
        }, 'Failed to download subtitles, falling back to Whisper transcription');
